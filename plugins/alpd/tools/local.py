...
def source_skill(root: Path) -> Path:
    skill = root / "plugins" / "alpd" / "skills" / "alpd"
    for path in (
        skill / "SKILL.md",
        skill / "scripts" / "alpd.py",
        root / "interaction-design-mvp" / "src" / "interaction_design" / "cli.py",
    ):
        if not path.is_file():
            raise ValueError(f"incomplete ALPD checkout: {path}")
    return skill
...
def destination(project: Path, host: str) -> Path:
    ...
    return project / HOSTS[host] / "skills" / "alpd"
...
def install(root: Path, project: Path, host: str) -> dict:
    ...
    return {
        ...
        "skill": str(target),
        "source": str(source),
        "environment": {"ALPD_PROJECT_ROOT": str(root)},
...
def doctor(
    ...
    *,
    ...
    command = [sys.executable, str(skill / "scripts" / "alpd.py"), "--project-root", str(root)]
    if cli is not None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        parser.exit(2, f"alpd local: {error}\n")
