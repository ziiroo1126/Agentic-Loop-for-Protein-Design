...
import { createToolOptions, ALPDToolError, resolveConfig, shellQuote } from './bridge.mjs';

const projectRoot = '/project/ALPD';
const workdir = `${projectRoot}/interaction-design-mvp`;
const cli = `${workdir}/.venv/bin/alpd`;
const submission = () => ({
...
function fixture(response = completed(), config = {}) {
  ...
  const tools = Object.fromEntries(createToolOptions(ctx, resolveConfig({ projectRoot, ...config }))
    .map((tool) => [tool.name.replace('alpd_screen_', ''), tool]));
  return { calls, contexts, controller, exec, tools, concludes: () => concludes };
}

async function rejectsCode(promise, code) {
  await assert.rejects(promise, (error) => error instanceof ALPDToolError && error.code === code);
}

test('configuration uses explicit root before environment and resolves executable paths', () => {
  assert.deepEqual(resolveConfig({}, { ALPD_PROJECT_ROOT: '/env/root/' }), {
    projectRoot: '/env/root', workdir: '/env/root/interaction-design-mvp',
    cli: '/env/root/interaction-design-mvp/.venv/bin/alpd', timeoutMs: undefined,
  });
  assert.equal(resolveConfig({ projectRoot, cli: '../other/bin/tool' }, { ALPD_PROJECT_ROOT: '/ignored' }).cli,
    '/project/ALPD/other/bin/tool');
...
test('configuration fails early for missing or relative roots, invalid paths, raw commands and bad timeouts', () => {
  ...
  ]) assert.throws(() => resolveConfig(config, {}), ALPDToolError);
});
...
test('POSIX quoting covers apostrophes, shell substitutions, newlines and empty strings', () => {
  ...
  assert.throws(() => shellQuote('a\0b'), ALPDToolError);
});
...
test('all execution identity, cancellation, foreground mode and contexts cross the host boundary', async () => {
  ...
  assert.equal(calls[0].callId, 'outer-call:alpd:observe:bash');
...
test('help describes the workflow without spawning work or inventing actor identity', async () => {
  const f = fixture();
  const result = await f.tools.alpd_help.execute({}, f.exec);
...
test('pipeline prepare passes full task/runtime paths and defaults without launching another command', async () => {
  ...
  assert.deepEqual(await f.tools.alpd_pipeline_prepare.execute({ task: 'tasks/binder.json', runtime: 'config/pipeline.json' }, f.exec), receipt);
...
test('pipeline prepare supports deterministic strategies, bounded options and safely quoted artifact paths', async () => {
  for (const strategy of ['fixed', 'heuristic']) {
    ...
    await f.tools.alpd_pipeline_prepare.execute({ task: '/task.json', runtime: '/runtime.json', strategy,
...
test('pipeline run and observe preserve prepared, pending and completed state envelopes', async () => {
  for (const action of ['run', 'observe']) {
    for (const payload of [
      ...
      assert.deepEqual(await f.tools[`alpd_pipeline_${action}`].execute({ pipeline_dir: "--pipeline'; $(id)\n`id`" }, f.exec), payload);
      assert.equal(f.calls[0].arguments.command,
...
test('pipeline apply uses the existing full submission over stdin and returns the next state', async () => {
  ...
  assert.deepEqual(await f.tools.alpd_pipeline_apply.execute({ pipeline_dir: '/pipeline', submission: proposal }, f.exec), payload);
  assert.equal(f.calls[0].arguments.command,
...
test('pipeline export quotes both directory arguments and returns the canonical export receipt', async () => {
  ...
  const f = fixture(completed(JSON.stringify(receipt)));
  assert.deepEqual(await f.tools.alpd_pipeline_export.execute({ pipeline_dir: 'artifacts/run', output: "--export's $(id)\n`id`" }, f.exec), receipt);
  assert.equal(f.calls[0].arguments.command,
...
test('pipeline tools reject unknown keys, missing paths, invalid budgets and malformed submissions before dispatch', async () => {
  ...
  for (const args of badPrepare) await rejectsCode(f.tools.alpd_pipeline_prepare.execute(args, f.exec), 'INVALID_ARGUMENTS');
  for (const action of ['run', 'observe']) {
    for (const args of [{}, { pipeline_dir: null }, { pipeline_dir: '/pipeline', command: 'arbitrary' }]) {
      await rejectsCode(f.tools[`alpd_pipeline_${action}`].execute(args, f.exec), 'INVALID_ARGUMENTS');
    }
  }
  const malformed = submission(); delete malformed.actor.model;
  await rejectsCode(f.tools.alpd_pipeline_apply.execute({ pipeline_dir: '/pipeline', submission: malformed }, f.exec), 'INVALID_ARGUMENTS');
  await rejectsCode(f.tools.alpd_pipeline_apply.execute({ pipeline_dir: '/pipeline', submission: submission(), command: 'arbitrary' }, f.exec), 'INVALID_ARGUMENTS');
  for (const args of [{ pipeline_dir: '/pipeline' }, { pipeline_dir: '/pipeline', output: null }, { pipeline_dir: '/pipeline', output: '/export', overwrite: true }]) {
    await rejectsCode(f.tools.alpd_pipeline_export.execute(args, f.exec), 'INVALID_ARGUMENTS');
  }
  assert.equal(f.calls.length, 0);
});
...
test('pipeline prepare and export reject malformed JSON receipts and incomplete directory paths', async () => {
  for (const [action, args, good] of [
    ...
    for (const stdout of invalid) {
      const f = fixture(completed(stdout));
      await rejectsCode(f.tools[`alpd_pipeline_${action}`].execute(args, f.exec), 'INVALID_CLI_OUTPUT');
    }
  }
});
...
test('pipeline execution shares the host identity, cancellation and context boundary', async () => {
  ...
  await f.tools.alpd_pipeline_run.execute({ pipeline_dir: '/pipeline' }, f.exec);
  assert.equal(f.calls[0].callId, 'outer-call:alpd:pipeline_run:bash');
  ...
  await rejectsCode(cancelled.tools.alpd_pipeline_run.execute({ pipeline_dir: '/pipeline' }, cancelled.exec), 'ABORTED');
  assert.equal(cancelled.calls.length, 0);
  const interrupted = fixture(completed('{}', { timedOut: true }));
  await rejectsCode(interrupted.tools.alpd_pipeline_apply.execute({ pipeline_dir: '/pipeline', submission: submission() }, interrupted.exec), 'TIMED_OUT');
