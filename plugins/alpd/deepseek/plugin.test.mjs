...
test('official defineTool compiles every registration, nested requiredness and output rendering', () => {
  ...
  assert.deepEqual(registered.map((tool) => tool.name),
    ['alpd_screen_prepare', 'alpd_screen_observe', 'alpd_screen_apply',
      'alpd_pipeline_prepare', 'alpd_pipeline_run', 'alpd_pipeline_observe',
      'alpd_pipeline_apply', 'alpd_pipeline_export', 'alpd_help']);
  ...
  assert.deepEqual(pipeline.alpd_pipeline_prepare.parameters.required, ['task', 'runtime']);
  assert.deepEqual(pipeline.alpd_pipeline_prepare.parameters.properties.strategy.enum, ['harness', 'fixed', 'heuristic']);
  assert.deepEqual(pipeline.alpd_pipeline_apply.parameters.required, ['pipeline_dir', 'submission']);
  assert.deepEqual(pipeline.alpd_pipeline_apply.parameters.properties.submission, schema.properties.submission);
  assert.deepEqual(pipeline.alpd_pipeline_export.parameters.required, ['pipeline_dir', 'output']);
...
test('official defineTool validates pipeline arguments before host execution', async () => {
  ...
  await assert.rejects(named.alpd_pipeline_prepare.execute({ task: '/task' }, {}), /runtime/);
  await assert.rejects(named.alpd_pipeline_prepare.execute({ task: '/task', runtime: '/runtime', strategy: 'other' }, {}), /strategy|enum/);
  await assert.rejects(named.alpd_pipeline_apply.execute({ pipeline_dir: '/pipeline', submission: { request_sha256: 'a'.repeat(64) } }, {}), /decision|actor/);
  await assert.rejects(named.alpd_pipeline_export.execute({ pipeline_dir: '/pipeline' }, {}), /output/);
  assert.equal(calls, 0);
});
