#!/usr/bin/env python3
"""Launch an existing local ALPD CLI without installing or downloading anything."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--project-root",
        default=os.environ.get("ALPD_PROJECT_ROOT"),
        help="ALPD repository root; defaults to ALPD_PROJECT_ROOT",
    )
    parser.add_argument(
        "--cli",
        default=os.environ.get("ALPD_CLI"),
        help="existing CLI executable path; defaults to ALPD_CLI or the local .venv",
    )
    parser.add_argument("cli_arguments", nargs=argparse.REMAINDER)
    options = parser.parse_args()
    if not options.project_root:
        parser.error("provide --project-root or ALPD_PROJECT_ROOT")
    ...
    if not (workflow / "pyproject.toml").is_file() or not (
        ...
        parser.error(f"not a ALPD checkout: {project_root}")

    if options.cli:
        executable = Path(options.cli).expanduser().resolve()
    else:
        executable = workflow / ".venv" / "bin" / "alpd"
        if not executable.is_file():
            located = shutil.which("alpd")
            if located is None:
