...
`alpd validate TASK.json` after authoring. Unsupported fields are errors.
...
```bash
alpd task init --name user_design --output /path/to/new/brief.json
alpd task review /path/to/new/brief.json
alpd task build /path/to/new/brief.json --output /path/to/new/task.json
```
