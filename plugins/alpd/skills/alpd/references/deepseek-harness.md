...
ALPD executor for evaluation, budget enforcement and recorded results.
...
`plugins/alpd/skills/alpd/SKILL.md` is not under a default root. DSH does not
...
you choose, for example `/absolute/path/to/alpd-skills.patch.yml`:

```yaml
- id: skill-filesystem
  config:
    customSkillDirs:
      - /absolute/path/to/ALPD/plugins/alpd/skills
```

The configured path is the parent containing the `alpd` skill directory. The
...
```bash
dsh --profile web --patch /absolute/path/to/alpd-skills.patch.yml --dump-config
```
...
ALPD_PROJECT_ROOT=/absolute/path/to/ALPD \
  dsh --profile web --patch /absolute/path/to/alpd-skills.patch.yml --no-open
```

Invoke `/alpd`, or ask the host to load the `alpd` skill, and supply the existing
session or preparation inputs. For a standalone installation, copying the complete
`skills/alpd/` directory into a chosen project's `.agents/skills/alpd/` also
...
plugins/alpd/
  ...
  cordis.patch.yml
  deepseek/index.mjs
  skills/alpd/SKILL.md
```

`package.json` declares `dsh.bundle.patch`; the referenced patch inserts the native
plugin row for the `dsh-alpd` package. DSH uses this npm bundle contract, rather than
...
```bash
dsh plugin --profile web add /absolute/path/to/ALPD/plugins/alpd
dsh --profile web --dump-config
```

DSH forwards the first command to pnpm, which must be available on `PATH`. The dump
should include the ALPD layer and its plugin row; it confirms configuration
composition, not execution of a tool or a model decision. Restart that profile after
adding or updating a bundle. Launch it with `ALPD_PROJECT_ROOT` pointing at the
...
| Field | Meaning |
| --- | --- |
| `projectRoot` | Absolute ALPD checkout path; defaults to `ALPD_PROJECT_ROOT`. |
| `cli` | Existing CLI executable path; a relative path resolves under `<projectRoot>/interaction-design-mvp`. Defaults to that directory's `.venv/bin/alpd`. |
| `timeoutMs` | Optional positive timeout passed to the host's `bash` tool. The host retains its default and cap when omitted. |

For example, an additional overlay can configure the installed `alpd` row:

```yaml
- id: alpd
  config:
    projectRoot: /absolute/path/to/ALPD
    cli: /absolute/path/to/existing/alpd
```
...
| --- | --- |
| `alpd_pipeline_prepare` | `task`, `runtime` JSON paths; optional `strategy`, `batch_size`, `max_evaluations`, `tool_wall_budget_seconds`, `artifacts`. Validate and prepare; return `status: prepared` and `pipeline_dir`. |
| `alpd_pipeline_run` | `pipeline_dir`. Run generation and monomer checks, then prepare complex screening. Harness returns `awaiting_selection`; fixed/heuristic complete automatically. |
| `alpd_pipeline_observe` | `pipeline_dir`. Read prepared state, pending request or completed report. |
| `alpd_pipeline_apply` | `pipeline_dir`, full `submission`. Evaluate/stop and return the next pipeline state. |
| `alpd_pipeline_export` | `pipeline_dir`, `output`. Export completed results and return `status: exported`, `pipeline_dir` and `export_dir`. |
...
| Tool | Operation |
| --- | --- |
| `alpd_screen_prepare` | Prepare screening from a completed monomer job and explicit replay or live inputs. |
| `alpd_screen_observe` | Return the current visible request or the completed report. |
| `alpd_screen_apply` | Submit a decision against the current request and let the executor validate it. |
| `alpd_help` | Return workflow guidance without running an evaluation. |
...
| Archived native screening execution | 10 checks using official Cordis, ToolRuntime, SystemPrompt, `tool-bash`, `bash-local` and `subprocess-local`, with the real ALPD Python CLI. |
...
It mounted the ALPD module through `ctx.plugin` and executed a CPU replay of
...
In the ALPD checkout, the reproducible integration script is
...
The dependency-free transport tests can run from the ALPD repository root:

```bash
node --test plugins/alpd/deepseek/bridge.test.mjs
```

With the pinned peer packages already available, `npm test` from `plugins/alpd`
