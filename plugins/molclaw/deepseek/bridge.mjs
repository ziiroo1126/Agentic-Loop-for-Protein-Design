...
export class MolClawToolError extends Error {
  constructor(code, message) {
    super(message);
    this.name = 'MolClawToolError';
    this.code = code;
  }
}

function fail(code, message) {
  throw new MolClawToolError(code, message);
}
...
export function resolveConfig(config = {}, env = process.env) {
  exactKeys(config, ['projectRoot', 'cli', 'timeoutMs'], [], 'MolClaw configuration');
  const projectRoot = pathString(config.projectRoot === undefined ? env.MOLCLAW_PROJECT_ROOT : config.projectRoot, 'projectRoot or MOLCLAW_PROJECT_ROOT');
  if (!isAbsolute(projectRoot)) {
    fail('INVALID_CONFIGURATION', 'projectRoot or MOLCLAW_PROJECT_ROOT must be an absolute MolClaw checkout path.');
  }
  const workdir = resolve(projectRoot, 'interaction-design-mvp');
  const cli = resolve(workdir, pathString(config.cli === undefined ? '.venv/bin/interaction-design' : config.cli, 'cli'));
...
function interrupted(exec) {
  if (exec.signal.aborted) fail('ABORTED', 'MolClaw call was cancelled. Inspect the session before submitting further work.');
}
...
function cliFailure(value) {
  if (value.sandbox?.denied || value.sandbox?.runnerFailed) {
    return 'The host sandbox denied or could not run MolClaw. Review the host policy result before continuing.';
  }
  ...
  if (value.exitCode === 126 || value.exitCode === 127 || /No such file or directory|Permission denied/.test(diagnostic)) {
    return 'Check projectRoot, cli, input paths, executable permissions, and the MolClaw virtual environment in the host bash result.';
  }
...
async function runCli(ctx, config, exec, action, command) {
  ...
  try {
    result = await ctx.tools.execute({
      callId: `${exec.callId}:molclaw:${action}:bash`,
      rootCallId: exec.rootCallId,
      name: 'bash',
      arguments: {
        command,
        description: {
          prepare: 'Prepare a bounded MolClaw screening session',
          observe: 'Read the current MolClaw screening observation',
          apply: 'Apply an evidence based MolClaw screening decision',
          pipeline_prepare: 'Validate and prepare a MolClaw design pipeline',
          pipeline_run: 'Generate candidates, evaluate monomers and start bounded complex screening',
          pipeline_observe: 'Read the current MolClaw pipeline status and visible screening request',
          pipeline_apply: 'Apply a visible-evidence decision to the MolClaw design pipeline',
          pipeline_export: 'Export a completed MolClaw pipeline result bundle',
        }[action],
  ...
  if (result?.isError !== false) {
    ...
    if (code === 'UNKNOWN_TOOL') fail('HOST_EXECUTION_FAILED', 'The host bash tool is unavailable. Enable the standard DeepSeek Harness bash tool for MolClaw.');
    if (code === 'ABORTED' || code === 'ABORTED_BEFORE_DISPATCH') {
      fail('ABORTED', 'The host cancelled the MolClaw call. Inspect the session before submitting further work.');
    }
    fail('HOST_EXECUTION_FAILED', 'The host bash tool rejected or failed the MolClaw call. Review its policy or execution diagnostics; this call was not retried.');
  }
  if (result.concludesTurn === true) exec.concludeTurn();
  const value = result.value;
  if (!record(value) || value.kind !== 'foreground') {
    fail('INVALID_HOST_RESULT', 'MolClaw requires a completed foreground bash result. Inspect any returned background job before continuing.');
  }
  if (value.aborted === true) fail('ABORTED', 'The host aborted MolClaw. Inspect preserved session work before continuing.');
  if (value.timedOut === true) fail('TIMED_OUT', 'MolClaw exceeded the host bash timeout. Inspect preserved session work and the configured host timeout before continuing.');
  if (value.aborted !== false || value.timedOut !== false || value.signal !== null) {
    fail('INVALID_HOST_RESULT', 'MolClaw did not finish normally or the host result lacks completion metadata. Inspect the host bash result.');
  }
  if (value.exitCode !== 0 || value.sandbox?.denied || value.sandbox?.runnerFailed) {
    ...
    fail('CLI_FAILED', `MolClaw ${action} failed${status}. ${cliFailure(value)}`);
  }
  if (!record(value.stdout) || typeof value.stdout.text !== 'string' || value.stdout.truncated !== false) {
    fail('INCOMPLETE_STDOUT', 'MolClaw stdout is missing or truncated. Inspect the full host output and increase its output limit before continuing.');
  }
...
function parseOutput(stdout) {
  ...
  try { value = JSON.parse(stdout); } catch {
    fail('INVALID_CLI_OUTPUT', 'MolClaw did not return complete JSON. Check the CLI version and host bash diagnostics.');
  }
  const pending = [value];
  while (pending.length) {
    const item = pending.pop();
    if (typeof item === 'number' && !Number.isFinite(item)) {
      fail('INVALID_CLI_OUTPUT', 'MolClaw returned a non-finite JSON number. Inspect the CLI output.');
    }
    if (item !== null && typeof item === 'object') {
      for (const child of Object.values(item)) pending.push(child);
    }
  }
  if (!record(value)) fail('INVALID_CLI_OUTPUT', 'MolClaw must return a JSON object. Check the CLI version.');
  return value;
}
...
function pipelineReceipt(stdout, status, paths) {
  ...
  if (value.status !== status || paths.some((key) => typeof value[key] !== 'string'
    ...
    fail('INVALID_CLI_OUTPUT', `MolClaw pipeline ${status} must return its documented status and absolute directory paths. Check the CLI version and host bash diagnostics.`);
  }
  return value;
}
  ...
  return [
    {
      ...
      async execute(args, exec) {
        ...
        if (!isAbsolute(sessionDir) || /[\r\n\0]/.test(sessionDir)) {
          fail('INVALID_CLI_OUTPUT', 'MolClaw prepare must return one absolute session directory. Check the CLI version and host bash diagnostics.');
        }
        return { session_dir: sessionDir };
      },
    },
    {
      name: 'molclaw_screen_observe',
    ...
    {
      name: 'molclaw_help',
      description: 'Explain the full MolClaw task → prepare → run → select → export workflow, existing screening tools, and actor provenance requirements.',
      parameters: {},
      output,
      async execute(args) {
