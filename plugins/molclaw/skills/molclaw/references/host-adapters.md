...
any model/API cost. Provide the MolClaw project root explicitly after loading a cached
...
checkout supplies `plugins/molclaw/tools/local.py`. From the checkout root:
...
an existing different file, directory or symlink. It prints `MOLCLAW_PROJECT_ROOT` for
...
`skills/molclaw/SKILL.md` and use it for the supplied session. For automatic local skill
discovery, copy `skills/molclaw/` into the selected project's `.agents/skills/molclaw/`,
then invoke `$molclaw`. These are documented
...
> Use $molclaw with MOLCLAW_PROJECT_ROOT=/absolute/path/to/MolClaw. Resume the screening
...
MOLCLAW_PROJECT_ROOT=/absolute/path/to/MolClaw \
  claude --plugin-dir /absolute/path/to/MolClaw/plugins/molclaw
```

Invoke `/molclaw:molclaw` and supply the session or preparation inputs. Local
...
```bash
claude plugin validate /absolute/path/to/MolClaw/plugins/molclaw --strict
```
...
`plugins/molclaw/skills` directory is **not discovered automatically**. Configure that
...
`molclaw_pipeline_apply`, `molclaw_pipeline_export`, and the existing
`molclaw_screen_prepare`, `molclaw_screen_observe`, and `molclaw_screen_apply`, using
the host's `bash` tool to reach the existing Python CLI; `molclaw_help` returns workflow
...
has been tested with the real MolClaw CLI and saved-result replay. That validation used
